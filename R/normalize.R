...
#'
#' @param G a weighted adjacency matrix
#' @param normalize string: type of normalization
#'
#' @return A weighted adjacency matrix
#'
#' @noRd
.normalize <- function(G, normalize) {
  
  #### Neighborhood rank, from Satuluri et al. (2011) ####
  if (normalize == "rank" | normalize == "embeddedness") {
    for (i in 1:nrow(G)) {  #For each row (i.e., from the perspective of each node)
      x <- G[i,]  #Vector of values from this row
  ...
  if (normalize == "embeddedness") {  #Scores will already be transformed as neighborhood ranks
    ...
    for (row1 in 1:(nrow(G)-1)) {
      for (row2 in (row1+1):nrow(G)) {  #Loop over each pair of rows
        list1 <- G[row1,-c(row1,row2)]  #Vector of ranked edges for row1, excluding row1 and row2
        list2 <- G[row2,-c(row1,row2)]  #Vector of ranked edges for row2, excluding row1 and row2
