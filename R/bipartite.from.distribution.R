...
#' B <- bipartite.from.distribution(R = 100, C = 100, P = 0.1, rowdist = c(1,10), coldist = c(1,10))  #Right-tailed
#' B <- bipartite.from.distribution(R = 100, C = 100, P = 0.1, rowdist = c(10,1), coldist = c(10,1))  #Left-tailed
#' B <- bipartite.from.distribution(R = 100, C = 100, P = 0.1, rowdist = c(10,10), coldist = c(10,10))  #Normal
#' B <- bipartite.from.distribution(R = 100, C = 100, P = 0.1, rowdist = c(10000,10000), coldist = c(10000,10000))  #Constant
  ...
  integers <- function(N, S, a, b){  
    for (i in 1:500000) {  #Attempt up to 500000 times
      if (a < b) {  #For right-tailed distributions
        ...
        plus <- rbeta(N,a,b)  #Distribution of amount to add to the floor
      ...
      if (a >= b) {  #For symmetric and left-tailed distributions
        rand <- rbeta(N,a,b)  #Distribution of values
