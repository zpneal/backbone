...
#' @param trials numeric: If trials > 1, this represents the number of bipartite graphs generated from the sampled distribution. If 0<trials<1, the number of graphs generated will be \eqn{.5(R^2-R)/trials + 1} where `R` is the number of rows.
#'     This option is to guarantee enough trials are ran if the user later intends to use a family-wise error rate correction in \link{backbone.extract}.
...
fdsm <- function(B,
  ...
  if ((trials > 0)&(trials<1)){
    trials <- ((.5*(dim(B)[1])**2-.5*(dim(B)[1]))/trials) + 1
  }
