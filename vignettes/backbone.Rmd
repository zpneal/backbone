...
5. [Advanced functions](#advanced)
    a. [Returned objects](#advanced-return)
...
## Welcome {#welcome}
This vignette illustrates how to use this package to extract the *backbone* of a network. A backbone is a sparse and unweighted subgraph of a network that contains only the most 'important' or 'significant' edges. A backbone can be useful when the original network is too dense, when edge weights are not needed, or when edge weights are difficult to interpret. 
...
For ease of illustration, this vignette also makes use of functions in the `igraph` package:
```{r}
library(igraph)
```
...
* You begin with some network data `dat`, which may take the form of an an adjacency/incidence matrix or an igraph object.

* Depending on the type of network contained in `dat` (bipartite, weighted, unweighted), you run the applicable backbone function (`backbone_from_bipartite()`, `backbone_from_weighted()`, or `backbone_from_unweighted()`), which returns a backbone of the same class as `dat`.

The sections below describe and illustrate the backbone functions applicable to different types of networks, including [weighted bipartite projections](#projection), [non-projection weighted networks](#weighted), and [unweighted networks](#unweighted). Additionally, the package contains some [utility functions](#utility) that are used by the backbone methods, but may also have independent applications.

## The `narrative` parameter {#narrative}
Each backbone extraction function includes a `narrative` parameter. When `narrative = TRUE` (the default), narrative text describing the backbone extraction will be displayed with the relevant citations. This text is suitable for use in manuscripts, and ensures complete and consistent reporting of the backbone extraction.

## Supported data formats {#formats}
The functions in the backbone package support two data formats that can be used to represent a network or graph in R:

* **matrix** - A bipartite network can be represented as an incidence matrix, and a unipartite matrix can be represented as an adjacency matrix. An incidence/adjacency matrix can be supplied to a backbone extraction function as a *matrix* object. It does not support *Matrix* objects from the `Matrix` package. You can convert a *Matrix* to a *matrix* using `as.matrix()`.

* **igraph** - A network can be supplied to a backbone extraction function as an *igraph* object from the `igraph` package.

The backbone package does not support *edgelist* formats because these can be ambiguous when isolated nodes are present. It also does not support *network* objects from the `statnet` package because these require loading many dependencies. You can convert a *network* bipartite graph into an incidence matrix using `network::as.matrix.network(graph, type = "incidence")`, and can convert a *network* unipartite graph into an adjacency matrix using `network::as.matrix.network(graph, type = "adjacency")`.
...
Although it is possible to visualize this network by using thicker lines to represent stronger edges, in larger networks this often leads to cluttered visualizations. Additionally, many network analytic techniques (e.g., ERGM, SOAM) have not yet been extended for application to weighted. In such cases, it can be useful to focus on this network's backbone.
```{r}
...
We could choose the threshold value based on the edge weights. For example, we could find the average edge weight, and use this value as the threshold to extract a backbone that preserves the stronger-than-average edges. However, this yields a network that is focused only on high-degree nodes (i.e., the large hub airport), while ignoring the low-degree nodes (i.e., the smaller hub airport):
```{r}
mean_weight <- mean(E(W)$weight)
bb <- backbone_from_weighted(W, model = "global", parameter = mean_weight)
plot(bb, vertex.label = NA)
```
...
Two additional statistical models are also available: locally adaptive network sparsification using `model = "lans"` [@foti2011] and marginal likelihood filter using `model = "mlf"` [@dianati2016]. Despite slight differences in their null models, they operate similarly to the disparity filter.

# Weighted bipartite projection networks {#projection}
## Background [@neal2014backbone] {#proj-background}
A bipartite network contains two types of nodes, which we call *agents* and *artifacts*, and edges that exist only between nodes of different types (i.e., an edge may connect an agent to an artifact, but never an agent to another agent). A bipartite network can be represented by an incidence matrix **B**, where B~ik~ = 1 if agent *i* is connected to artifact *k*, and otherwise is 0 (e.g., author *i* wrote paper *k*).

A bipartite projection is a network of *agents* that are connected by the fact that they share *artifacts* in common. A bipartite projection can be represented by a square symmetric matrix **P**, which is formed as **P** = **B** * **B**', where **B**' is the transpose of **B**. In **P**, P~ij~ indicates the number of artifacts *k* that are shared by agents *i* and *j*, and can be viewed as the weight of the edge connecting them in the network (e.g., the number of papers co-authored by authors *i* and *j*), however these weights can be noisy. The goal of backbone extraction from bipartite projections is to identify edges in **P** whose weights are strong and should be preserved in the backbone. 
...
All of the backbone models implemented in the backbone package for bipartite projections are statistical models, which choose edges to return in the backbone by testing whether their weights are statistically significantly stronger than expected under a null model, with the stringency of the statistical test controlled by the `alpha` parameter. Among the fastest and most widely used backbone models is the Stochastic Degree Sequency Model (SDSM) [@neal2021comparing].

We can extract an SDSM backbone at a conventional level of statistical significance using the `backbone_from_bipartite()` function and specifying `model = "sdsm"` and `alpha = 0.05`. Importantly, bipartite projection backbones are based on the original bipartite network, not on the weighted projection. Therefore, we supply `B` and not `P` to the `backbone_from_bipartite()` function. Because this model uses information from the original bipartite network to identify strong edges, it preserves the network's true three-community structure.
```{r}
bb <- backbone_from_bipartite(B, model = "sdsm", alpha = 0.05)
plot(bb, vertex.label = NA)
```

Four additional statistical models are also available: fixed degree sequence model (FDSM) using `model = "fdsm"` [@zweig2011systematic], fixed row model using `model = "fixedrow"`, fixed column model using `model = "fixedcol"`, and fixed fill model using `model = "fixedfill"` [@neal2021comparing]. Each of these models uses different null models that constrain different features of `B` when evaluating edges' strength. The SDSM and FDSM are often most useful, but have trade-offs. The FDSM offers more statistical power, but requires computationally expensive monte carlo methods to estimate edges p-values. In contrast, the SDSM is less powerful (although this can be compensated by using a more liberal `alpha'), but can quickly compute edges' exact p-values.

[back to Table of Contents](#toc)

# Unweighted networks {#unweighted}
## Background {#unweight-background}
An unweighted unipartite network contains one type of node, and pairs of nodes are either connected or not. An unweighted unipartite network can be represented by an adjacency matrix **U** where U~ij~=1 if nodes *i* and *j* are connected, and otherwise is 0. Unweighted networks can be undirected (i.e., W~ij~ = W~ji~) or directed (i.e., W~ij~ != W~ji~), but currently the models implemented in the backbone package only allow undirected networks.

The goal of backbone extraction from an unweighted network is to identify edges in **U** that are important to the network's structure, and thus should be preserved in the backbone. Because the network is already unweighted, and the goal is simply to reduce the number of edges, this process is sometimes also called *sparsification*. The challenge is that, unlike [bipartite projections](#projection) and [weighted networks](#weighted), the edges in unweighted networks do not have weights that can be used to evaluate their importance. To overcome this challenge, backbone models designed for unweighted networks first assign each edge a score that is intended to capture its importance.

[back to Table of Contents](#toc)

## Structural models {#unweight-structural}
All of the backbone models implemented in the backbone package for unweighted networks are structural models. Sifferent models are designed to preserve or uncover different structural features of the network, but all are characterized by key decisions:
...
Other models, such as the Local Degree model [@hamann2016structure], are designed to uncover hubs. This model involves assigning each edge a score using the degree of an endpoint node, normalizing these scores by ranking them from the perspective of each node, and filtering them based on the degree of each node. To illustrate, we can generate a dense network `U` that contains some hubs, then extract its backbone using the `backbone_from_unweighted()` function and specifying `model = "degree"` and `parameter = 0.5`. The backbone preserves the highest-degree nodes as hubs.
```{r fig.width=8, out.width="75%"}
par(mfrow = c(1, 2), mar = c(0,0,2,0))  #Display plots side-by-side
U <- sample_pa(n = 60, m = 3, directed = FALSE)  #Example unweighted network
plot(U, main = "Original Network", vertex.size = degree(U))
bb <- backbone_from_unweighted(U, model = "degree", parameter = 0.5)
plot(bb, main = "Local Degree Backbone", vertex.size = degree(bb))  #Communities are clearly visible
```

Eight additional structural models are also available: Skeleton backbone using `model = "skeleton"` [@karger1994random], Global sparsification using `model = "gspar"` [@satuluri2011local], Simmelian backbone using `model = "simmelian"` [@nick2013simmelian], Jaccard backbone using `model = "jaccard"` [@goldberg2003assessing], Meetmin backbone using `model = "meetmin"` [@goldberg2003assessing], Geometric backbone using `model = "geometric"` [@goldberg2003assessing], hypergeometric backbone using `model = "hyper"` [@goldberg2003assessing], and Quadrillateral Simmelian backbone using `model = "quadrilateral"` [@nocaj2015untangling]. Each of these models involve different ways of scoring, normalizing, and filtering edges.

[back to Table of Contents](#toc)

# Advanced functions {#advanced}
## Returned objects {#advanced-return}
By default, `backbone_from_` functions return a backbone in the same class (matrix or igraph) as the original network. However, to facilitate transparency and reproducibility, specifying `return = "everything"` returns a list that contains:

* The backbone, in the same class as the original network
* The original network
* The weighted projection (for `backbone_from_bipartite()`)
...
bb <- backbone_from_bipartite(B, model = "sdsm", alpha = 0.05, return = "everything")  #Extract backbone, return everything
...
B <- rbind(cbind(matrix(rbinom(250,1,.8),10),  #Incidence matrix of example bipartite network
           ...
           cbind(matrix(rbinom(250,1,.2),10),
                 ...
                 matrix(rbinom(250,1,.8),10)))
B <- graph_from_biadjacency_matrix(B)  #Convert to an igraph object
bb1 <- backbone_from_bipartite(B, model = "sdsm", alpha = 0.05)  #Extract non-signed backbone
layout <- layout_nicely(bb1)
plot(bb1, vertex.labels = NA, layout = layout, main = "Non-signed")
bb2 <- backbone_from_bipartite(B, model = "sdsm", alpha = 0.1, signed = TRUE)  #Extract signed backbone
E(bb2)$color <- "green"
...
Extracting a backbone from a bipartite projection using the fixed degree sequence model with `backbone_from_bipartite(model = "fdsm")` requires randomly sampling matrices from the space of all binary matrices with given row and column sums, which is performed using the fastball algorithm [@godard2021fastball]. Given a matrix, `fastball()` returns a new matrix randomly sampled from the space of all binary matrices with the same row and column sums:
```{r}
