...
The `backbone.extract()` function has four parameters: `matrix`, `signed`, a significant test value `alpha`, and `fwer`. The `matrix` parameter takes in the entire backbone object which is the output of `hyperg(), fdsm(),` or `sdsm()`. If the `signed` parameter is set to `TRUE` (the default) a signed backbone is returned, if `FALSE` a binary backbone is returned.

One can adjust the precision of the significance test, `alpha`, to refine their backbone results. The value of `alpha` should be between `0` and `1`. The default is `alpha=0.05`. If `signed = FALSE`, then the `alpha` value is equal to the user's input, and the statistical test is one-tailed yielding a binary backbone. If the `signed = TRUE`, the `alpha` value is equal to the user's input divided by two, and the statistical test is two-tailed yielding a signed backbone. 

The `fwer` parameter is set by default to `NULL` but can take two character values: "holm" if Holm Bonferroni Family-wise Error Rate test should be used; or "bonferroni" if Bonferroni Family-wise Error Rate test should be used.

If an entry in the `positive` matrix is greater than the `alpha` value, it is considered a `+1` edge in the backbone. If an entry in the `negative` matrix is greater than the `alpha` value, it is considered a `-1` edge in the backbone. All other values are `0` in the backbone graph. The `backbone.extract()` function will return a backbone graph of the same class as the input object in `hyperg()`, `sdsm()`, or `fdsm()`, i.e. a matrix, sparse matrix, igraph object, edgelist, or network object. 
...
* positive: a matrix with $(i,j)$ entry equal to the probability that $G_{ij}^*$ is equal to or above the corresponding entry in $G$.
* negative: a matrix with $(i,j)$ entry equal to the probability that $G_{ij}^*$ is equal to or below the corresponding entry in $G$.
...
hyperg_bb <- backbone.extract(hyperg_probs, signed = TRUE)
```
...
```{r}
sdsm_bb <- backbone.extract(sdsm_rna, signed = FALSE, alpha = 0.1, fwer = "bonferroni") 
sdsm_bb
```
...
```{r echo=T, results='hide'}
fdsm_props <- fdsm(davis, trials = 100, dyad=c(1,5))
```
```{r}
fdsm_props$dyad_values
fdsm_bb <- backbone.extract(fdsm_props, signed = TRUE, alpha = 0.1)
fdsm_bb
```
