...
The `backbone.extract()` function has three parameters: `matrix`, `signed`, and a significant test value `alpha`. The `matrix` parameter takes in the entire output of `hyperg(), fdsm(),` or `sdsm()`. If the `signed` parameter is set to `TRUE` (the default) a signed backbone is returned, if `FALSE` a binary backbone is returned.
...
The `sdsm( )` function has five parameters,
